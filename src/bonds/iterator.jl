...
env_cutoff(cutoffs::AbstractDict{Tuple{AtomicNumber,AtomicNumber},CUTOFF}) where {CUTOFF<:AbstractBondCutoff} = maximum(env_cutoff(c) for c in values(cutoffs))
...
function _get_bond_env(iter::BondsIterator, i, j, rrij)
    ...
    rrmid = rri + 0.5 * rrij
    Js = Int[]
    sizehint!(Js, length(Js_i) ÷ 4)
    Rs = typeof(rrij)[]
    sizehint!(Rs, length(Js_i) ÷ 4)
    Zs = AtomicNumber[]
    sizehint!(Zs, length(Js_i) ÷ 4)
...
Alternatively, indsf can also be of the form of a filter function `atom_filter(i::Int,at::AbstractAtoms)::Bool`, that returns `true` if bonds to the ith atom
   ...
   where both atoms satisfy the filter criterion.
"""
function bonds(at::Atoms, cutoffs::AbstractDict{Tuple{AtomicNumber,AtomicNumber},CUTOFF}, subset::Array{<:Int}) where {CUTOFF<:AbstractBondCutoff}
...
function bonds(at::Atoms, cutoffs::AbstractDict{Tuple{AtomicNumber,AtomicNumber},CUTOFF}, filter=_ -> true) where {CUTOFF<:AbstractBondCutoff}
    subset = findall(i -> filter(i, at), 1:length(at))
